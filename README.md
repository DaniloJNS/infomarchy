...
One card per running agent — **Claude Code, Codex, Grok, Grok Bot, Gemini, Hermes, opencode, aider, Ollama chats** — detected straight from `/proc`, no agent-side hooks, nothing to configure. Each card shows the project, working directory, how long it's been up, the pid, the workspace it lives on, and the terminal's own title. Its two-line **current topic** is a short synopsis derived from several exact-session requests—not the last prompt copied onto the card. A loaded local Ollama model may refine the wording; summaries are cached by session/content and Infomarchy never auto-loads a model. The dot **pulses while the agent is thinking** — and that, like *blocked / waiting / done*, is now a **reported** state rather than a guess wherever the agent runs inside **Herdr**: Infomarchy reads Herdr's own `agent.list` over the same socket it already uses to focus panes (4-6 ms, no subprocess), joined to the session by pane id. Herdr's `working` drives the dot; its `blocked` means *an approval or question is on screen* and becomes **waiting**; its `done` means *finished while you were not looking*, and since Herdr tracks seen-ness itself — and a read like this one does not mark a tab seen — the signal survives polling and clears when you click the card. A pane Herdr cannot classify (`unknown`), and any agent it does not host, falls back to the terminal-title heuristics described below. Merge conflicts outrank all of it: they come from git, not from a string.
...
**The strip is a horizontal carousel.** Cards are a fixed width on one scrolling row rather than squeezed to fit: with six sessions the old single row gave each card ~213 px and elided every line, including the topic — the only line that tells one session from another. Scroll it with the mouse wheel (a vertical wheel scrolls sideways), drag the track beneath it, or press **J/K** in the overlay, which scrolls the strip to keep the selected card visible. The edges fade where the strip continues and the header counts what is on screen (`1–3 of 6`). Nothing rotates on its own. Cards are ordered by **urgency** — waiting or blocked, then finished-but-unseen, then working, then idle — so the only thing the carousel can push out of sight is an idle session; within a group the order is newest first.

Each card is five lines: state, provider and uptime; project; topic; git; host. The working directory duplicated the project, the window title duplicated the topic, and the `pid · cpu · ram · proc · gpu` line repeated the MACHINE card, so all three live in the right-click inspector. There is no duplicate compact workspace-card strip; the inspector carries the workspace number. In the session inspector, workspace buttons 1–10 can move that exact agent window silently; the current workspace is highlighted and disabled.
...
### 🟣 GitHub · you — *what is waiting on you*

Under the heatmap row, in two independently scrolling columns.

**MY PRS** is every open pull request you authored, newest change first: how long ago it moved, a **DRAFT** tag, the repository without its owner, the number, the issue key lifted out of the title into its own tag (`[INFLTECH-14351] Um caminho…` becomes a tag plus a title that spends its width on words), and at the end the CI verdict — `✓` passing, `✗` failing, `·` no checks or none reported yet — with **REVIEW** when GitHub is still waiting on reviewers. Drafts are included because a draft *is* an open PR on GitHub.

**REVIEW REQUESTS** is one list of everything pointed at you: first the pull requests waiting on your review, then a rule, then your unread notifications, each with a short tag for its reason (**CI**, **MENTION**, **REVIEW**, **ASSIGN**, …). The card header counts `N open · N draft · N CI✗ · N unread`.

Left-click any row to open it in the browser; the overlay closes behind it. That is the only thing the card does — it is deliberately read-only and ambient. Nothing here marks a notification read, requests a review or posts a comment; the [`robzolkos.github`](https://github.com/robzolkos/omarchy-github-plugin) bar plugin is still where you act on any of it. In the overlay this module has no number key — it is the eleventh, and **1–9, 0** reach the first ten — so its chip in the strip is how you toggle it.

Data comes through `gh`, in two calls issued together: one GraphQL document covering both pull request lists, and unread `/notifications`. Both are slimmed by `gh --jq`, so no pull request body is ever parsed. The three lists are cached whole in a private state file written by the wallpaper collector and read by the overlay, so summoning the overlay with **SUPER+D** costs no API calls; they refresh every five minutes and back off exponentially while failing. A failed call leaves the last known lists on the card rather than emptying it, marked *stale* once failures have persisted fifteen minutes. Without `gh`, or before `gh auth login`, the header says exactly that. `INFOMARCHY_SKIP_GITHUB=1` disables the fetch.
...
- **`collector.ts`** builds one snapshot. It reads `argv` for every pid (cheap), then lazily opens only agent processes and their ancestors, so a 1 000-process box costs ~0.2 s warm. Local files are opened once with no-follow/nonblocking semantics, must be regular files, and are read under byte/time limits. Rate baselines use private, atomic state files under `$XDG_STATE_HOME/infomarchy/prev-<instance>.json`, and the two GitHub caches (`github-activity.json`, `github-inbox.json`) are written 0600 by the wallpaper collector and read by the overlay. It never parses the multi-hundred-MB Claude/Codex session transcripts — only the small history/index files and OpenCode's local SQLite history.
