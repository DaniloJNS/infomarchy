...
### Added
- **LIVE AI SESSIONS is a horizontal carousel.** It used to be a single non-wrapping row of four to six cards, because a second row pushed RECENT TASKS off a 1080p desk — so with six sessions each card got ~213 px and every line elided, including the topic, which is the only line that tells one session from another (four of six cards just said "hermes"). Now it is one scrolling strip of fixed-width cards: a vertical mouse wheel scrolls it sideways, the track under it is draggable, the edges fade where the strip continues, and the header counts which cards are on screen (`1–3 of 6`). **J/K** in the overlay scrolls the strip to keep the selected card visible, since a selection that steps out of sight makes the key look broken. Nothing rotates on its own — periodic movement on a wallpaper steals attention all day and slides the card out from under the pointer.
- **Sessions are ordered by urgency**, which a carousel makes a correctness requirement rather than a nicety: waiting or blocked first, then work finished but unseen, then what is still running, then idle — so the only thing the strip can ever push out of sight is an idle session. Within a group the order is unchanged (newest first, pid as the tiebreak), so cards keep a stable place across ticks instead of swapping under the cursor.
- **Live session state comes from Herdr, not from a window title.** The desk used to infer everything about a session by pattern-matching the terminal title: the pulsing "thinking" dot from `/Processing|🧠|⚙|⏳|…/`, and blocked / waiting / done from a dozen regexes over the same string. Herdr already does real detection, and Infomarchy already talks to its socket to focus panes, so it now also reads `agent.list` (4-6 ms, no subprocess, one call per tick, and only when something is actually Herdr-hosted). Herdr's `working` drives the dot; its `blocked` — "an approval or question UI is on screen" — becomes the desk's **waiting**, not its *blocked*, which stays reserved for a real failure; its `done` becomes **done**, and because Herdr tracks *seen* itself (and a CLI read does not mark a tab seen) polling cannot erase that signal while clicking the card, which focuses the pane, clears it. `unknown` never reads as healthy: it falls back to the old title heuristics, as does any session Herdr does not host. Merge conflicts still outrank Herdr, because they come from git rather than from a string. This feeds the session card, **NEXT ACTIONS** and the notification pipeline alike, so expect *more* alerts than before — Herdr sees prompts the regex never did.
- `herdr-status.ts`: `agent.list` over the socket `herdr-focus.ts` already opens, joined to sessions on **`pane_id`** (the `HERDR_PANE_ID` the collector reads from `/proc/<pid>/environ`) and deliberately **not** on `agent_session.value` — that id drifts, verified live: a session that had been through `/rewind` reported its pre-rewind id to Herdr while the plugin read the current one, so joining there would invent a phantom card on every rewind or resume. Bounded like the rest: 600 ms budget (a fifth of the focus path's, since a hung socket must not cost the tick), 128 agents, 180-character titles, every reply shape-checked, and a failed or garbled read simply leaves the session on its own signals for that tick.
- **GITHUB · YOU.** A full-width card under the heatmap row, in two scrolling columns: **MY PRS** — every open pull request you authored, with its age, a **DRAFT** tag, the repository without its owner, the number, the issue key lifted out of the title into its own tag, the title itself, and the CI verdict (`✓` passing, `✗` failing, `·` none or not yet reported) plus **REVIEW** when GitHub is still waiting on reviewers — and **REVIEW REQUESTS** — the pull requests waiting on *your* review, then a rule, then your unread notifications, each tagged with its reason (**CI**, **MENTION**, **REVIEW**, …). The header counts `N open · N draft · N CI✗ · N unread`. Left-click any row to open it in the browser (the overlay closes behind it). Read-only on purpose: nothing here marks a notification read or requests a review — the bar's GitHub plugin is still where you act. It is a removable module, and the eleventh, so it has no number key; the chip in the strip toggles it and **1–9, 0** still reach the first ten.
- `github-inbox.ts`: one GraphQL document for `viewer.pullRequests(states: OPEN)` (drafts included — draft is a flag on an open PR, not a state) and `search(review-requested:@me)`, plus unread `/notifications`, both issued in parallel and slimmed by `gh --jq` so no pull request body is ever parsed (~1.3 s and ~0.6 s measured). A private `github-inbox.json` store, written by the wallpaper collector and read by the overlay, so summoning the overlay costs no API calls; five-minute refresh with the heatmap's exponential backoff, a 6 s command budget because GitHub's search backend has been seen past 3 s, and the same *missing / unauthenticated / pending / unavailable / stale / ok* states so the two GitHub cards never disagree about whether `gh` works. A failed call keeps the last known lists rather than emptying the card. `subject.url` is null for whole classes of notification (a failed CheckSuite has no single API object), so those rows fall back to the repository page, and an API URL is rewritten to the web one. `INFOMARCHY_SKIP_GITHUB=1` disables it.
...
### Changed
- **The session card is five lines.** Status word, provider and uptime; project; topic; git; host. The working directory duplicated the project, the window title duplicated the topic, and the `pid · cpu · ram · proc · gpu` line repeated what MACHINE already reports — all three now live in the right-click inspector, which gained the session's registry `name` so nothing is lost. The card is also shorter than it was, which hands vertical budget back to RECENT TASKS below (four rows to six on this desk). The **STALE** tag replaces the status word rather than sitting beside "IDLE", and keeps how long the session has been idle.
- Claude's own registry status and the systemd turn inhibitor still outrank Herdr for *busy*: both are facts about a running turn, where Herdr reports what it can see on screen.
- `sendHerdrRequest` is now a thin wrapper over `sendHerdrCommand`, which returns the reply's `result` instead of a boolean. `Bun.connect` throws synchronously on a malformed socket path (an empty `unix` is an `ERR_INVALID_ARG_TYPE`, not a connection error), which rejected the promise rather than resolving it; that is caught, because the collector now calls this on every tick.
- Demo mode's session roster covers all four Herdr states and no longer points **NEXT ACTIONS** at a hardcoded roster index.
...
- The hand-drawn scrollbar RECENT TASKS grew — wheel travel from anywhere over the list, a draggable track, a thumb sized to the content — is now a `ScrollList` component shared by RECENT TASKS and both GITHUB · YOU columns, instead of a fourth copy of the same sixty lines.
